...
jest.mock("../data/database", () => {
  const doc = { _id: "507f1f77bcf86cd799439011", title: "Mock Comic" };
  return {
    getDatabase: () => ({
      db: () => ({
        collection: () => ({
...
describe("GET /comics/:id", () => {
  it("400 on invalid id", async () => {
    const res = await request(app).get("/comics/not-a-valid-id");
    expect(res.status).toBe(400);
  });

  it("200 on valid id", async () => {
    const id = "507f1f77bcf86cd799439011";
    const res = await request(app).get(`/comics/${id}`);
    expect(res.status).toBe(200);
    expect(res.body.title).toBe("Mock Comic");
  });
});
